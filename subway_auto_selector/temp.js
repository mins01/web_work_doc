...
$('.pd_list_wrapper >ul>li').each(
    function(idx,el){
        ...
        var img = $(this).find(".img img").map(retfn).attr('src');
        console.log(img);
        for(var i=0,m=tit.length;i<m;i++){
            r.push(['cat',this.className,tit[i],eng[i],(cal[i]?cal[i]:''),(summary[i]?summary[i]:''),(img[i]?img[i].replace('../','http://subway.co.kr/'):'')]);
        }
    }
)
